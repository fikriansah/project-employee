...
const DetailUser = () => {
    ...
    const refreshToken = async () => {
        try {
            ...
            const detailUsers = await axiosJWT.get(`http://localhost:5000/users/${id}`, {
                headers: {
    ...
    return (
        <div className="container mt-5">
            ...
            <Link to={`/dashboard/user/edit/${id}`} className='button is-small is-success mr-2'>EDIT</Link>
            <button className='button is-small is-danger'
            onClick={() => deleteUsers(id)}
            >DELETE</button>
            <div className="fixed-grid has-3-cols has-text-white mt-5">
                <div className="grid">
